...
const DEFAULT_SETTINGS = {
  id:'logistics_settings', minOrdersPerRoute:2, maxOrdersPerRoute:3, groupingWindowMinutes:3,
...
async function getGroupableOrders() {
  const snap = await db.collection('orders').where('status', '==', 'waiting_grouping').limit(300).get();
...
function buildSug(chunk, basis, maxKm) {
  ...
  return {
    ...
    basis, maxKm: basis === 'proximidade' ? Math.round(dmax * 10) / 10 : null,
...
// Sugestões: por DISTÂNCIA real (quem tem coordenada) + fallback por bairro. NÃO persiste.
app.get('/api/routes/suggestions', async (_req, res) => {
  ...
  // 1) Agrupamento por proximidade (greedy: começa pelo mais antigo, junta os mais próximos dentro do raio)
  const used = new Set();
  for (const seed of withGeo) {
    if (used.has(seed.id)) continue;
    used.add(seed.id);
    const group = [seed];
    const cands = withGeo.filter(o => !used.has(o.id))
      .map(o => ({ o, d: haversineKm(seed.address, o.address) }))
      .filter(x => x.d <= maxKm).sort((a, b) => a.d - b.d);
    for (const c of cands) { if (group.length >= max) break; group.push(c.o); used.add(c.o.id); }
    if (group.length >= min) suggestions.push(buildSug(group, 'proximidade', maxKm));
...
app.post('/api/grouping/tick', async (_req, res) => {
  ...
  res.json({ autoGroup: !!s.autoGroup, windowMin, loneMin, created, lone });
});
...
app.post('/api/routes', async (req, res) => {
  ...
  if (orderIds.length > max) return err(res, 'TOO_MANY', 400, `Máximo de ${max} pedidos por rota (regra R2)`);
...
app.patch('/api/drivers/:id/status', async (req, res) => {
  ...
  else { patch.arrivedAtStore = null; patch.isInsideStoreGeofence = false; }
  await ref.set(patch, { merge: true });
  res.json((await ref.get()).data());
});
...
function queueSort(a, b) {
  const ai = a.isInsideStoreGeofence ? 0 : 1, bi = b.isInsideStoreGeofence ? 0 : 1;
  if (ai !== bi) return ai - bi;
  const at = (a.isInsideStoreGeofence ? a.arrivedAtStore : a.wentOnlineAt) || '';
  const bt = (b.isInsideStoreGeofence ? b.arrivedAtStore : b.wentOnlineAt) || '';
  return at < bt ? -1 : 1;
}
async function queueList() {
  const snap = await db.collection('drivers').where('online', '==', true).get();
  return snap.docs.map(d => d.data()).filter(d => !d.activeRouteId).sort(queueSort);
}
...
async function doAssign(routeId, driver, orderIds) {
  ...
  batch.set(db.collection('drivers').doc(driver.id), { activeRouteId: routeId, status: 'route_suggested', updatedAt: now }, { merge: true });
  await batch.commit();
}
...
app.post('/api/routes/:id/assign', async (req, res) => {
  ...
  batch.set(db.collection('drivers').doc(driverId), { activeRouteId: req.params.id, status: 'route_suggested', updatedAt: now }, { merge: true });
  await batch.commit();
