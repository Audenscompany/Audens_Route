...
(function(){
  ...
  const IC={
    ...
    estoque:S('<path d="M3 8l9-4 9 4-9 4-9-4Z"/><path d="M3 8v8l9 4 9-4V8"/><path d="M12 12v8"/>'),
  };
  const LOGO={
  ...
  const NAV=[['home','Dashboard','dashboard.html','dashboard'],['orders','Pedidos','pedidos.html','pedidos'],['group','Agrupamento','agrupamento.html','agrupamento'],['mapa','Mapa','mapa.html','mapa'],['route','Rotas','rotas.html','rotas'],['moto','Motoboys','motoboys.html','motoboys'],['chart','Métricas','metricas.html','metricas'],['cash','Financeiro','financeiro.html','financeiro'],['hist','Histórico','historico.html','historico'],['estoque','Estoque','estoque-contagem.html','estoque'],['store','Lojas','cardapios.html','lojas'],['users','Usuários','usuarios.html','usuarios'],['gear','Configurações','configuracoes.html','config']];
  ...
  const api={
    ...
    // ---- Estoque / Contagem / Compras ----
    stockProducts:(store,activeOnly)=>j(API+'/api/stock/products?store='+encodeURIComponent(store||'')+(activeOnly?'&active=true':'')),
    stockCreateProduct:(b)=>j(API+'/api/stock/products',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)}),
    stockPatchProduct:(id,b)=>j(API+'/api/stock/products/'+id,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)}),
    stockDelProduct:(id)=>j(API+'/api/stock/products/'+id,{method:'DELETE'}),
    stockCategories:(store)=>j(API+'/api/stock/categories?store='+encodeURIComponent(store||'')),
    stockCreateCategory:(b)=>j(API+'/api/stock/categories',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)}),
    stockPatchCategory:(id,b)=>j(API+'/api/stock/categories/'+id,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)}),
    stockDelCategory:(id)=>j(API+'/api/stock/categories/'+id,{method:'DELETE'}),
    stockSuppliers:(store)=>j(API+'/api/stock/suppliers?store='+encodeURIComponent(store||'')),
    stockCreateSupplier:(b)=>j(API+'/api/stock/suppliers',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)}),
    stockPatchSupplier:(id,b)=>j(API+'/api/stock/suppliers/'+id,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)}),
    stockDelSupplier:(id)=>j(API+'/api/stock/suppliers/'+id,{method:'DELETE'}),
    stockOpenCount:(storeId,by,day)=>j(API+'/api/stock/counts/open',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({storeId,by,day})}),
    stockGetCount:(id)=>j(API+'/api/stock/counts/'+id),
    stockSaveCount:(id,b)=>j(API+'/api/stock/counts/'+id,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)}),
    stockFinalizeCount:(id,b)=>j(API+'/api/stock/counts/'+id+'/finalize',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b||{})}),
    stockCountsHistory:(store)=>j(API+'/api/stock/counts/history?store='+encodeURIComponent(store||'')),
    stockLists:(store,day)=>j(API+'/api/stock/lists?store='+encodeURIComponent(store||'')+(day?('&day='+day):'')),
    stockList:(id)=>j(API+'/api/stock/lists/'+id),
    stockPatchList:(id,b)=>j(API+'/api/stock/lists/'+id,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)}),
    stockDupList:(id)=>j(API+'/api/stock/lists/'+id+'/duplicate',{method:'POST'}),
